class CleanGameLogs:
    def __init__(self, df): # pass df result from get_game_logs
        ...
        self.raw_df = df.rename(columns={'team_abbreviation': 'team'})
        self.raw_df.to_csv('raw.csv', index=False)
        
        # call funcs for subset dfs
        self.active_players()
        self.active_teams()
        self.player_box()
    ...
    def player_box(self):
        self.player_box_df = self.raw_df[['game_id', 'player_id', 'team_id', 
                                          'min', 'pts', 'ast', 'reb', 'stl', 'blk', 'oreb', 'dreb', 'tov', 'pf']]
    ...
    def team_box(self):
        self.team_box_df = self.raw_df[['game_id', 'team_id', 'team', 'min', 'pts', 'ast', 'reb',
